...
export class P9Server {
  ...
  constructor(backend?: FsaBackend) { this.backend = backend ?? null; }
  async setRoot(root: FileSystemDirectoryHandle, policy: readonly WorkspaceCapability[] = ['read']): Promise<void> { this.backend = await FsaBackend.attach(root, policy); this.fids.clear(); this.qids.clear(); this.transaction = null; }
  setTransactionPolicy(policy: TransactionPolicy | null): void { this.transaction = policy ? { transactionId: policy.transactionId, capabilities: Object.freeze([...policy.capabilities]) } : null; this.backend?.setPolicy(this.transaction?.capabilities ?? ['read']); }
  ...
  async handle(frame: Uint8Array, reply: (response: Uint8Array) => void): Promise<void> {
    ...
    try {
      ...
      if (request.type === 'Tflush') { this.suppressedTags.add(request.oldtag); this.respond({ type: 'Rflush', tag: request.tag }, reply); return; }
  ...
  private async dispatch(request: P9Request): Promise<P9Response> {
    ...
    switch (request.type) {
      ...
      case 'Tlcreate': { const fid = this.fid(request.fid); const handle = await backend.createFile(fid.pathSegments, request.name); fid.pathSegments = [...fid.pathSegments, request.name]; fid.openMode = request.flags; return { type: 'Rlcreate', tag: request.tag, qid: await this.qid(fid.pathSegments, handle.kind), iounit: 0 }; }
      ...
      case 'Twrite': { const fid = this.fid(request.fid); this.reserveMutation(request.data.byteLength); try { return { type: 'Rwrite', tag: request.tag, count: await backend.write(fid.pathSegments, toSafeBrowserNumber(request.offset), request.data) }; } finally { this.queuedMutationBytes -= request.data.byteLength; } }
      ...
      case 'Tsetattr': { const fid = this.fid(request.fid); if (request.valid & 0x8) await backend.truncate(fid.pathSegments, toSafeBrowserNumber(request.size)); return { type: 'Rsetattr', tag: request.tag }; }
      ...
      case 'Tmkdir': { const dir = this.fid(request.fid); const created = await backend.mkdir(dir.pathSegments, request.name); return { type: 'Rmkdir', tag: request.tag, qid: await this.qid([...dir.pathSegments, request.name], created.kind) }; }
      case 'Trenameat': { const oldDir = this.fid(request.olddirfid); const newDir = this.fid(request.newdirfid); await backend.rename(oldDir.pathSegments, request.oldname, newDir.pathSegments, request.newname); return { type: 'Rrenameat', tag: request.tag }; }
      case 'Tunlinkat': { const dir = this.fid(request.dirfid); await backend.remove(dir.pathSegments, request.name, (request.flags & 0x200) !== 0); return { type: 'Runlinkat', tag: request.tag }; }
    }
