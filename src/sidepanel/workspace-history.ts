...
export class WorkspaceHistoryStore {
  ...
  private async database(root: FileSystemDirectoryHandle) {
  ...
  async append(root: FileSystemDirectoryHandle, record: WorkRecord): Promise<void> {
    const { db, file } = await this.database(root);
  ...
  async load(root: FileSystemDirectoryHandle): Promise<readonly WorkRecord[]> {
    const { db, file } = await this.database(root);
    try { const result = db.exec('SELECT id, created_at, provider, task, outcome, status FROM work_history ORDER BY created_at DESC'); const rows = result[0]?.values ?? []; await this.persist(db, file); return rows.map(([id, createdAt, provider, task, outcome, status]) => ({ id: String(id), createdAt: Number(createdAt), provider: provider as WorkRecord['provider'], task: String(task), outcome: String(outcome), status: status as WorkRecord['status'] })); } catch (error) { db.close(); throw error; }
  }
