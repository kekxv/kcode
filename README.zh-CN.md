...
`public/v86/` 中的内核、initramfs 和 v86/BIOS 是扩展启动与 CI 必需的锁定资源，已提交到 Git，使干净 checkout 可以验证和构建。SquashFS 根文件系统嵌入 initramfs，不再作为独立重复文件分发；CI 会从 initramfs 解出它后验证大小和安全性。
